use std::{collections::HashMap, convert::Infallible, ops::AddAssign, vec};

use proc_macro::{Delimiter, Group, Ident, Literal, Span, TokenStream, TokenTree};
use typed_quote::{IntoTokenTree, IntoTokens, ToTokens, WithSpan, quote, tokens::IterTokens};
...
use crate::{
    ...
    ident_match,
    syn_generic::{
        GroupBracket, GroupParen, MetaAfterPath, NoPayload, Parse, ParseError, Parseable,
        PunctComma, PunctEq, StructData,
...
impl StructAttrs {
    pub fn parse(
            ...
            StructData::Paren { paren, semi } => {
                ...
                let res = ParsingTokenStream::from(paren.stream()).parse_into_unnamed_fields(
                    ...
                    |_, attrs, ty, comma| {
                        ...
                        let f = StructField::from_attrs(attrs, name, ty);
                        fs.push(f);
                    },
                );
        ...
        ContextOfStruct {
            ...
            self_dot: None,
            to: PropDefaultCustom::new(to_default, to.map(CustomTokens::from)),
...
impl StructField {
    ...
    fn from_attrs(
        ...
        StructField {
            ...
            to: PropExpanded::new(to.map(From::from)),
            to_kvs: PropDefaultCustom::new(to_kvs_default, to_kvs.map(From::from)),
            to_items: PropDefaultCustom::new(to_items_default, to_items.map(From::from)),
        }
    }
}
...
enum StructFieldToItemsDefault {
    BracketedTo,
    Flatten { span: Span },
}
